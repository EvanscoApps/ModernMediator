...
- The notification dispatch routing distinction persists in v2.2 even though `ErrorPolicy` and `HandlerError` participation is now unified. `IPublisher.Publish<TNotification>(notification, cancellationToken)` invokes DI-resolved `INotificationHandler<TNotification>` instances; the `IMediator.Publish<T>(T)` and `IMediator.PublishAsync<T>(T, CancellationToken)` overloads invoke runtime `Subscribe<T>` and `SubscribeAsync<T>` callbacks. A single publish call reaches one path, not both. The unification this ADR establishes is at the error-handling layer, not the dispatch layer.
